...
// (5) 달리는 토끼 A
const bunnyAImage = new Image();
bunnyAImage.src = "./images/bunny_running_a.png";
// (6) 달리는 토끼 B
const bunnyBImage = new Image();
bunnyBImage.src = "./images/bunny_running_b.png";
// (7) 게임 오버 토끼
const bunnyCrashImage = new Image();
bunnyCrashImage.src = "./images/bunny_crash.png";
// (8) 장애물
...
const bunny = {
    ...
    draw() {      // 달리는 애니메이션 구현
        if (gameOver) {
            ...
            // 달리는 애니메이션 구현
            if (timer % 60 > 30) {
                ctx.drawImage(bunnyAImage, this.x, this.y, this.width, this.height);
            } else {
                ctx.drawImage(bunnyBImage, this.x, this.y, this.width, this.height);
            }
        }
    },
};
...
function animate() {
    ...
    if (jump) {
        ...
        if (bunny.y < 400) {
            ...
            if (bunny.y > 400) bunny.y = 400; // bunny가 canvas 하단을 넘지 않도록 조정
        }
    }
    /** end of 토끼 */
}
...
/** 2-3 장애물 조건1 충돌함수 */
/** 충돌 체크 함수 */
function collision(bunny, obstacle) {
    return !(
        bunny.x > obstacle.x + obstacle.width/1.2 ||
        bunny.x + bunny.width/2 < obstacle.x |
        bunny.y > obstacle.y + obstacle.height/1.2 ||
        bunny.y + bunny.height/2 < obstacle.y
    );
}
